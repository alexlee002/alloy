...
- (void)dealloc
{
    ...
    if (userAgentString.retainCount > 0) {
...
- (void)buildRequestHeaders
{
	...
	if (![[self requestHeaders] objectForKey:@"User-Agent"]) {
		NSString *userAgentStringT = [self userAgentString];
		if (!userAgentStringT) {
			userAgentStringT = [ASIHTTPRequest defaultUserAgentString];
		}
		if (userAgentStringT) {
			[self addRequestHeader:@"User-Agent" value:userAgentStringT];
		}
	}
...
- (void)startRequest
{
	...
	if ([self shouldAttemptPersistentConnection]) {
		...
		if (![self connectionInfo]) {
			...
			[[self connectionInfo] setObject:@(nextConnectionNumberToCreate) forKey:@"id"];
	...
	// Schedule the stream
	if (![self readStreamIsScheduled] && [self hasThrottleWakeUpTime]) {
...
- (void)checkRequestStatus
{
	...
	if ([self readStream]) {
		...
		if ([self postLength]) {
			...
			if (stream != nil) {
				[self setTotalBytesSent:[NSMakeCollectable([(NSNumber *)CFReadStreamCopyProperty((CFReadStreamRef)stream, kCFStreamPropertyHTTPRequestBytesWrittenCount) autorelease]) unsignedLongLongValue]];
			}
...
- (void)requestFinished
{
	...
	if ([self isPACFileRequest] || self.needParseResponseAsynchronous) {
		[self reportFinished];
	} else {
...
- (void)handleBytesAvailable
{
	...
	bufferSize = [self bandwidthReadBufferSize:bufferSize];
	...
	// If zero bytes were read, wait for the EOF to come.
		...
		[self incrementReadStreamBytes:bytesRead];
...
- (BOOL)checkIsReallyDownCompleted:(NSError **)fileError{
  NSString *cLength = [responseHeaders valueForKey:@"Content-Length"];
  if (nil != cLength && nil != fileError ) {
    long long contentlenght = [[[self responseHeaders] objectForKey:@"Content-Length"] longLongValue];
    if (contentlenght != self.totalBytesRead) {
      //没有完全下载
      NSError *downfileError = [NSError errorWithDomain:NetworkRequestErrorDomain code:ASIDownNotEqualContentLenght userInfo:nil];
      *fileError = [NSError errorWithDomain:NetworkRequestErrorDomain code:ASIDownNotEqualContentLenght userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"Failed downfile contentlenght ='%@' and downlenght ='%@'",@(contentLength),@(self.totalBytesRead)],NSLocalizedDescriptionKey,downfileError,NSUnderlyingErrorKey,nil]];
      return NO;
    }
  }
  return YES;
...
{	
    ...
    //检查contentlenght是否与totalRead字符相等
    [self checkIsReallyDownCompleted:&fileError];
    if (nil != fileError) {
      //如果不相等,则抛出错误
...
- (void)scheduleReadStream
{
	if ([self readStream] && ![self readStreamIsScheduled]) {
		...
		[connectionsLock lock];
		runningRequestCount++;
		if (shouldUpdateNetworkActivityIndicator && ![self needHiddenIndicator]) {
...
- (BOOL)configureProxies
{
	...
	if (![self isPACFileRequest] && (![self proxyHost] && ![self proxyPort])) {
		...
		if ([self PACurl]) {
			...
			proxies = NSMakeCollectable([(NSArray *)CFNetworkCopyProxiesForURL((CFURLRef)[self url], (__bridge CFDictionaryRef)proxySettings) autorelease]);
