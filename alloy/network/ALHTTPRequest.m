...
- (void)requestDidSucceedWithResponse:(nullable ALHTTPResponse *)response {
    ...
    if (self.responseModelBlock != nil) {
        if (JSON != nil /*&& self.responseModelClass != nil*/) {
            ...
            ALLogWarn(@"(id:%ld) Can not parse model from response. reason: %@", (long) self.identifier,
                      JSON == nil ? @"response data is not in JSON format!"
                                  : (self.responseModelClass == nil ? @"response model class is not specified!"
                                                                    : @"unknown error"));
...
- (nullable id)modelByParsingResponseJSON:(in id)JSONObject error:(inout NSError **)error {
    ...
    if ([JSONObject isKindOfClass:[NSArray class]]) {
        result = [self.responseModelClass al_modelArrayWithJSON:JSONObject];
    } else {
        result = [self.responseModelClass al_modelWithJSON:JSONObject];
    }
